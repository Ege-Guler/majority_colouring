...
# Create one single environment for the entire run
const GRB_ENV = Gurobi.Env()

# Silence all Gurobi output for this environment
...
timers = Dict{String, Float64}(
    "mask_to_graph"         => 0.0,
    "is_weakly_connected"   => 0.0,
    "is_cyclic"             => 0.0,
    "solve_majority"        => 0.0,
    "is_isomorphic_to_seen" => 0.0,
    "connectivity"          => 0.0,
    "independence_number"   => 0.0,
    "clique_number"         => 0.0,
    "ham_path"              => 0.0
)
...
# --- Main enumeration ---
total       = 2^25 # Total 5-vertex directed graphs (2^(5*4))
MAX_IMAGES  = 200
...
open(output_csv, "w") do f
    ...
    for mask in 0:(total - 1)
        ...
        timers["mask_to_graph"] += @elapsed G = mask_to_graph(mask)
        ne_count = ne(G)

        timers["is_weakly_connected"] += (t_wc = @elapsed wc = is_weakly_connected(G))
        wc || continue
        
        timers["is_cyclic"] += (t_cyc = @elapsed cyclic = is_cyclic(G))
        cyclic && continue # Skip graphs with self-loops or multigraphs if they exist

        timers["solve_majority"] += @elapsed begin
            _, _, y_val, coloring = solve_majority_coloring_out(G, true, true, true)
        end

        chromatic = y_val === nothing ? -1 : sum(y_val)
        
        timers["is_isomorphic_to_seen"] += @elapsed iso       = is_isomorphic_to_seen(G)
        timers["connectivity"]          += @elapsed conn      = compute_vertex_connectivity(G)
        timers["independence_number"]   += @elapsed stability = compute_independence_number(G)
        timers["clique_number"]         += @elapsed clique    = compute_clique_number(G)
        timers["ham_path"]              += @elapsed ham_path  = has_hamiltonian_path(G)
...
if total_processed > 0
    println("\n  Average Time per Processed Graph:")
    for (task, duration) in sorted_timers
        avg = duration / total_processed
        println("    $(rpad(task, 25)): $(round(avg * 1000, digits=4)) ms")
    end
end
